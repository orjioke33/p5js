...
const SUN_COLOR = "	#FFE484";
const SUN_GLOW = "100";
const SUN_RADIUS = 70;

const SKY_TOP_COLOR = "	#2F9DE2";
const SKY_BOT_COLOR = "	#98BFDE";
const SKY_SHADE_COUNT = 4;
const SKY_STEP = 4;
const SKY_AMP = 30;
const SKY_ZOOM = 0.008;
const SKY_COLOR = "#B1E8FF";
const SKY_SEPERATION = 14;

const RIDGE_TOP_COLOR = "#b5be7f";
const RIDGE_BOT_COLOR = "#475f5f";
const RIDGE_COUNT = 6;
const RIDGE_STEP = 4;
const RIDGE_AMP = 150;
const RIDGE_ZOOM = 0.1;
const RIDGE_SEPERATION = 8;
...
function draw() {
  ...
  for (let k = 0; k < SKY_SHADE_COUNT; k++) {
    let skyTopHeight = k * (height / SKY_SEPERATION);
    drawSky(k+1, skyTopHeight);
  }
  drawSun(width/2, skyHeight - RIDGE_AMP / 2);
  
  for (let j = 0; j < RIDGE_COUNT; j++) {
    let ridgeHeight = (skyHeight+15) + (j * (height / RIDGE_SEPERATION));
